...
import { ITypeError } from "./IError";
import { Indexable } from "./Indexable";
import { Instance } from "./Instance";
import { Token } from "./Token";

export class ObjectFFI extends Instance {
  object: any;
  cache: Map<string, Callable | Instance> = new Map();

  constructor(object: any) {
    super(new IClass("ffi", new Map(), undefined));
    this.object = object;
  }
...
export class MethodFFI extends ObjectFFI implements Callable {
...
export class LangList extends ObjectFFI implements Indexable {
  ...
  private processKey(bracket: Token, key: any): number {
    if (typeof key !== "number") {
      throw new ITypeError(bracket, "Expect number for list index.");
    }
    if (key >= this.object.length || -key > this.object.length) {
      throw new ITypeError(bracket, "Index out of bounds.");
    }
...
class Append implements Callable {
  isCallable: true = true;
  list: any[];
